...
int V,E,t,f;
int flow[2*MAX_E],cap[2*MAX_E],to[2*MAX_E],next_edge[2*MAX_E],last[MAX_V];
int path[MAX_V-1],sz;
...
void dfs(int u, int depth){
    ...
    for(int v,num=last[u];num!=-1 && !found;num=next_edge[num]){
        v=to[num];
...
int main(){
    ...
    while(1){
        ...
        fill(next_edge,next_edge+2*MAX_E,-1);
        fill(last,last+V,-1);
