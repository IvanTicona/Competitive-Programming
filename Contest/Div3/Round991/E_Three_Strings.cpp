...
void solve(){
  ...
  for (int i = 0; i <= n; ++i){
    for (int j = 0; j <= m; ++j){
      ...
      if (i > 0 && c[k - 1] == a[i - 1]) dp[i][j] = min(dp[i][j], dp[i - 1][j]);
      else if (i > 0) dp[i][j] = min(dp[i][j], dp[i - 1][j] + 1);

      if (j > 0 && c[k - 1] == b[j - 1]) dp[i][j] = min(dp[i][j], dp[i][j - 1]);
      else if (j > 0) dp[i][j] = min(dp[i][j], dp[i][j - 1] + 1);
    }
  }
